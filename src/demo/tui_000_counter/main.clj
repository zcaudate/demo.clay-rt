(ns demo.tui-000-counter.main
  (:use code.test)
  (:require [std.lang :as  l]
            [std.lib :as h]))

(fact:global
 {:prelim  [(require 'demo.tui-000-counter.build)
            (eval (std.make/run:init demo.tui-000-counter.build/PROJECT))]
  :setup   [(eval (std.make/run:dev demo.tui-000-counter.build/PROJECT))]})
